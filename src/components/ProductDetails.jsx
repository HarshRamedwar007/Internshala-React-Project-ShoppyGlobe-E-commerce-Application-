...
//                 </p>
//                 <p className="flex items-center gap-x-2 mt-3 text-[22px] min-[500px]:text-2xl font-semibold">₹ {grandTotalAmount} <span className="text-base mt-1">({product.discountPercentage} % OFF)</span></p>
...
//                     className={`${!addedToCart ? "cursor-pointer opacity-100" : "cursor-not-allowed opacity-60"} mt-7 font-semibold text-[16px] min-[500px]:text-lg bg-purple-700 text-white py-2 px-4 rounded-lg`}>
    ...
    return (
        <section className="mt-10 min-[500px]:mt-20 min-[700px]:mt-28 px-5 flex flex-col min-[700px]:flex-row items-center min-[700px]:items-start gap-x-5">
            ...
            <section>
                ...
                <p className="flex items-center gap-x-2 mt-3 text-[22px] min-[500px]:text-2xl font-semibold">₹ {product?.price}</p>
                <section className="mt-5 font-semibold text-[20px]">
                    Description:
                    <ul className="mt-2 ml-5 block text-[18px] space-y-2 max-w-[600px]">
                        {product?.description.split('. ').map(sentence => <li key={Math.floor(Math.random() * 1000000)} className="text-[15px] min-[500px]:text-base list-disc">{sentence}</li>)}
